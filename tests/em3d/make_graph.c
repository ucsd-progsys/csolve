...
#define NUM_H_NODES  NNODES
#define H_NODE_DEGREE DNODES

#define NUM_E_NODES  NNODES
#define E_NODE_DEGREE DNODES
...
void fill_table(node_t * * COUNT(size) table, int size)
{
  int i;
  ...
  for (i=0; i<size; i++)
    {
      ...
      table[i]->value = (int) gen_uniform_double();
...
void make_neighbors(node_t *nodelist,
                    node_t ** COUNT(tablesz) table, int tablesz,
		    int degree)
{
  node_t *cur_node;
  ...
  for (cur_node = nodelist; cur_node; cur_node=cur_node->next)
    {
      ...
      cur_node->coeffs = (int *) ALLOC(from_count * sizeof(int));
      for (k=0; k<from_count; k++)
          cur_node->coeffs[k] = (int) gen_uniform_double();
...
graph_t initialize_graph()
{
  ...
  int num_h_nodes = NUM_H_NODES;
  int num_e_nodes = NUM_E_NODES;

  node_t * * COUNT(num_h_nodes) h_table;
  node_t * * COUNT(num_e_nodes) e_table;
  graph_t retval;
