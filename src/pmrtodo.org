...
* TODO Drop add_field_fold_overlap
...
* TODO Better error message when can't parse predicate
...
* TODO Change assert to lcc_assert

* TODO Figure out how to make assert, validptr compileable

* TODO Write docs for spec language

* TODO Show specs for externs without OKEXTERN

* TODO Delete all spec files
...
** TODO Relating argc to argv
   argv only contains argc-many non-NULL pointers. This looks like yet
   another use for contextual types:

   : LOC(argv) :: r: 0[4]: {v: ref (...) | r - BLOCK_BEGIN([v]) < 4 * argc => r != NULL}

   Note that this affects mst-scratch, ks, and probably some other large benchmarks.
