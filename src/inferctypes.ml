...
let ctypevar_of_ctype: ctype -> ctypevar = function
  | CTInt (n, i) -> CTInt (n, IEConst i)
  | CTRef (s, i) -> CTRef (s, IEConst i)

let ctypevar_fresh_indices: ctypevar -> ctypevar = function
  | CTInt (n, _) -> CTInt (n, IEVar (fresh_indexvar ()))
  | CTRef (s, _) -> CTRef (s, IEVar (fresh_indexvar ()))
...
type storecstr =
  | SCInc of S.t * indexexp * ctypevar (* (l, i, ct): (i, ct) in store(l) *)
...
let refine_store (l: Sloc.t) (ie: indexexp) (ctv: ctypevar) (is: indexsol) (ss: storesol): indexsol * storesol =
  match indexexp_apply is ie with
    | IBot   -> (is, ss)
    | IInt n ->
        let pl = PLAt n in
...
let storeinc_sat ((l, ie, ctv): S.t * indexexp * ctypevar) (is: indexsol) (ss: storesol): bool =
...
let mk_storeinc (loc: C.location) (l: Sloc.t) (ie: indexexp) (ctv: ctypevar): cstr =
  {cdesc = CSStore (SCInc (l, ie, ctv)); cloc = loc}
...
let mk_const_storeinc (loc: C.location) (l: Sloc.t) (i: index) (ct: ctype): cstr list =
  let ctv = ctypevar_of_ctype ct in
  let cti = ctypevar_fresh_indices ctv in
    [mk_storeinc loc l (IEConst i) cti; mk_subty loc ctv cti]
...
let refine (loc: C.location) ((is, ss): cstrsol): cstrdesc -> indexsol * storesol = function
  ...
  | CSStore (SCInc (l, ie, ctv))      ->
      try
        refine_store l ie ctv is ss
      with TypeDoesntFit ->
        let i  = indexexp_apply is ie in
        let ct = ctypevar_apply is ctv in
...
let constrain_const (loc: C.location): C.constant -> ctypevar = function
...
let constrain_sizeof (loc: C.location) (t: C.typ): ctypevar =
  CTInt (int_width, IEConst (IInt (CilMisc.bytesSizeOf t)))

let rec constrain_exp_aux (ve: ctvenv) (em: cstremap) (loc: C.location): C.exp -> ctypevar * cstremap * cstr list = function
  | C.Const c                     -> (constrain_const loc c, em, [])
  ...
  | C.SizeOf t                    -> (constrain_sizeof loc t, em, [])
...
and apply_unop (op: C.unop) (em: cstremap) (loc: C.location) (rt: C.typ) (ctv: ctypevar): ctypevar * cstremap * cstr list =
  match op with
    | C.LNot -> (CTInt (typ_width rt, IEConst (ISeq (0, 1))), em, [])
    | C.BNot -> (CTInt (typ_width rt, IEConst ITop), em, [])
    | C.Neg  -> (CTInt (typ_width rt, IEConst ITop), em, [])
...
and constrain_constptr (em: cstremap) (loc: C.location): C.constant -> ctypevar * cstremap * cstr list = function
  | C.CStr _ ->
      ...
      let ctv = fresh_ctvint char_width in
        (CTRef (s, IEConst (IInt 0)), em, [mk_storeinc loc s (IEConst (ISeq (0, 1))) ctv; mk_subty loc (CTInt (char_width, IEConst ITop)) ctv])
...
and constrain_cast (ve: ctvenv) (em: cstremap) (loc: C.location) (ct: C.typ) (e: C.exp): ctypevar * cstremap * cstr list =
  let ectv = constrain_exp_aux ve em loc e in
    match (C.unrollType ct, C.unrollType <| C.typeOf e) with
      | (C.TInt (ik, _), C.TPtr _) -> (CTInt (C.bytesSizeOfInt ik, IEConst ITop), em, [])
      | (C.TInt (ik, _), C.TInt _) ->
          begin match ectv with
            | (CTInt (n, ie), em, cs) ->
                let ie =
                  if n <= C.bytesSizeOfInt ik then
                    ...
                    IEConst ITop
                in (CTInt (C.bytesSizeOfInt ik, ie), em, cs)
      ...
      let (ctv, (ctvm, cs)) = constrain_exp ve em loc e in
        begin match ctv with
          | CTRef (s, iv) ->
              ...
              let cs    = mk_storeinc loc s iv ctvlv :: cs in
                (ctvlv, (ctvm, cs))
...
let instantiate_args (loc: C.location) (argcts: (string * ctype) list): ctypevar list =
  List.map (ctypevar_of_ctype <.> snd) argcts
...
let instantiate_function (loc: C.location) (env: ctypeenv) (f: string): ctypevar * ctypevar list * cstr list * RA.annotation list =
  ...
  let argctvs          = instantiate_args loc argcts in
  let rctv             = ctypevar_of_ctype rct in
  let sublocs          = List.map snd subs in
  let storecs          = mk_uniqlocs loc sublocs :: instantiate_store loc sout in
    (rctv, argctvs, storecs, List.map (fun (s1, s2) -> RA.New (s1, s2)) subs)
