...
let rec refctypeOfCilType abstr mem t = match normalizeType t with
...
and preRefcfunOfType t =
  ...
  let argrcts            = argts 
                      |>: (M.app_snd <| (refctypeOfCilType true SM.empty <+> RCt.subs sub)) in
  let retrct             = ret |> refctypeOfCilType true SM.empty |> RCt.subs sub in
  ...
  if not <| C.isFunctionType v.C.vtype then begin
    let _ = C.currentLoc := v.C.vdecl in
      M.sm_protected_add
        false
        v.C.vname
        (v.C.vtype |> refctypeOfCilType false SM.empty |> RCt.subs sub, specTypeOfVar v)
        varm
  end else
    varm
