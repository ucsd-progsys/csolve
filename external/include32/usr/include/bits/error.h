...
extern void __REDIRECT (__error_alias, (int __status, int __errnum,
					__const char * ARRAY VALIDPTR __format, ...),
			error)
  __attribute__ ((__format__ (__printf__, 3, 4))) OKEXTERN;
...
error (int __status, int __errnum, __const char * ARRAY VALIDPTR __format, ...) OKEXTERN
{
...
extern void __REDIRECT (__error_at_line_alias, (int __status, int __errnum,
						__const char * ARRAY VALIDPTR __fname,
						unsigned int __line,
						__const char * ARRAY VALIDPTR __format, ...),
			error_at_line)
  __attribute__ ((__format__ (__printf__, 5, 6))) OKEXTERN;
...
/* If we know the function will never return make sure the compiler
   realizes that, too.  */
__extern_always_inline void
error_at_line (int __status, int __errnum, __const char * ARRAY VALIDPTR __fname,
	       unsigned int __line,__const char * ARRAY VALIDPTR __format, ...) OKEXTERN
{
  if (__builtin_constant_p (__status) && __status != 0)
