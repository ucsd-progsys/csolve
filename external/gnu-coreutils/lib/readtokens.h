...
{
  size_t size;
  char * ARRAY NNREF(PVALIDPTR) NNREF(PSIZE_GE(size)) buffer;
};
typedef struct tokenbuffer token_buffer;

void init_tokenbuffer (token_buffer *tokenbuffer) OKEXTERN;

size_t REF(-1 <= V)
  readtoken (CSOLVE_IO_FILE_PTR stream,
             const char * ARRAY VALIDPTR delim,
             size_t n_delim,
