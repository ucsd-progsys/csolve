...
main (int REF(V > 0) argc, 
      char * ARRAY VALIDPTR LOC(PROGRAM_NAME_LOC) * START NONNULL ARRAY SIZE(argc * 4) argv)
  CHECK_TYPE GLOBAL(PROGRAM_NAME_LOC)
{
  uid_t uid;
  GETGROUPS_T * ARRAY gids = NULL;
  size_t n_gids = 0;
    ...
    while ((c = getopt_long (argc, argv, "+g:", NULL, NULL)) != -1)
      {
		...
		CSOLVE_ASSUME(optarg != NULL);
  		      ...
  		      return EXIT_FAILURE;
  	    		...
  	    		return SETUIDGID_FAILURE; // ABAKST
                      }
                    gr = ptr + 1;
                  }
                break;
...
/*   { */
/*     char *ARRAY *cmd = argv + optind + 1; */
/*     /\* csolve_assert(optind < argc); *\/ */
/*     int exit_status; */
/*     // ABAKST had to add that these two may alias? Is that ok or no? */
/*     /\* execvp (*cmd, cmd); *\/ */
