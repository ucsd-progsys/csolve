...
static int
set_additional_groups (char const * STRINGPTR groups)
{
  ...
  for (tmp = strtok (buffer, ","); tmp; tmp = strtok (NULL, ","))
    {
      struct group *g;
      // ABAKST when we constrain this dude with 'top', should we say
      // we know it is V > 0 (unsigned)?
  ...
  CHECK_TYPE GLOBAL(PROGRAM_NAME_LOC)
{
  ...
  /* ABAKST: need to alias optarg with argv */
  extern char * NNSTRINGPTR LOC(PROGRAM_NAME_LOC) optarg;
  ...
  if (argc == optind + 1)
    {
extern char * LOC(LCC_PROGRAM_NAME) NNSTRINGPTR getenv2 (__const char * STRINGPTR __name) __THROW __nonnull ((1)) __wur OKEXTERN;
      /* No command.  Run an interactive shell.  */
      char *shell = getenv2 ("SHELL");
      if (shell == NULL)
