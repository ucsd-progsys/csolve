  ...
  CHECK_TYPE GLOBAL(PROGRAM_NAME_LOC)
{
  ...
  // pmr: Annotation for the same reason as in unlink
  int pmr_optind = optind;

  if (argc < pmr_optind + 2)
    {
      if (argc < pmr_optind + 1)
        error (0, 0, _("missing operand"));
      else
        error (0, 0, _("missing operand after %s"), quote (argv[pmr_optind]));
      usage (EXIT_FAILURE);

      return 0;
    }

  if (pmr_optind + 2 < argc)
    {
      error (0, 0, _("extra operand %s"), quote (argv[pmr_optind + 2]));
      usage (EXIT_FAILURE);

      return 0;
    }

  if (link (argv[pmr_optind], argv[pmr_optind + 1]) != 0)
    error (EXIT_FAILURE, errno, _("cannot create link %s to %s"),
           quote_n (0, argv[pmr_optind + 1]), quote_n (1, argv[pmr_optind]));
