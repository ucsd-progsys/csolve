...
char * STRINGPTR csolve_check_pos(char * ARRAY p) CHECK_TYPE
{
 return p;
}
...
static char const NULLTERMSTR * LOC(L) STRINGPTR
decimal_absval_add_one (char NULLTERMSTR * STRINGPTR LOC(L) buf)
{
  bool negative = (buf[1] == '-');
  char *absnum = buf + 1 + negative;
  char *p = absnum + strlen (absnum);
  /* ABAKST: can only index with indices >= 0 */
  /* absnum[-1] = '0'; */
  *(absnum - 1) = '0';
  /* ABAKST the loop will terminate safely because there is a
     character not equal to '9'. But csolve doesn't know this.
     Maybe with contextual types? */
  while ((p > buf) && *(p = csolve_check_pos(p-1)) == '9')
     *p = '0';
  ++*p;
  char *result = MIN (absnum, p);
  if (negative) {
    CSOLVE_ASSUME(result > buf);
    *csolve_check_pos(--result) = '-';
  }
  return result;
}

int
main (int REF(V > 0) argc, char NULLTERMSTR * ARRAY VALIDPTR LOC(PROGRAM_NAME_LOC) * START NONNULL ARRAY SIZE(argc * 4) argv)
...
#define print_float(TYPE)                                                \
  ...
  print_int (OFF_T);
  print_int (INTMAX);
  /* ABAKST: CIL can't handle this number right now, seems to be a bug
