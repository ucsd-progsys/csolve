// pmr: See optind difficulties, which persist in spite of global
//      heapification
...
static bool
bsd_sum_file (const char NULLTERMSTR * ARRAY file, int print_name)
{
  FILE *fp;
...
static bool
sysv_sum_file (const char NULLTERMSTR * ARRAY file, int print_name)
{
  int fd;
    ...
    CHECK_TYPE GLOBAL(PROGRAM_NAME_LOC)
{
  ...
  int optc;
  int files_given;
  bool (*sum_func) (const char * STRINGPTR, int) = bsd_sum_file;
  ...
  if (files_given <= 0)
    ok = sum_func ("-", files_given);
      ...
      for (ok = true; optind < argc; optind++) {
          int z = optind;
#ifdef CIL
          int pmr_assm1 = csolve_assume (z < argc);
#endif
          ok &= sum_func (argv[z], files_given);
      }
