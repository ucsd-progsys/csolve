...
static void
remove_suffix (char * STRINGPTR LOC(L) name, const char * STRINGPTR LOC(L) suffix)
{
  char *np;
  const char *sp;
  ...
  //ABAKST: need to tell the index solver about information
  //        in grd
  while (np > name && sp > suffix)
    if (*csolve_check_pos(--np) != *csolve_check_pos(--sp))
      return;
  
  if (np > name)
    *csolve_check_pos(np) = '\0';
}
