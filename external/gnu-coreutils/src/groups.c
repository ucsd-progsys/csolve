  ...
  CHECK_TYPE GLOBAL(PROGRAM_NAME_LOC)
{
  ...
  if (optind == argc)
  ...
  else
    {
      ...
      // pmr: here, heapifying optind is not going to do us much good - or is it
      //      ok because of the prior desugarings
      while (optind < argc)
        {
          // pmr: Annotation
          int pmr_optind = optind;
          struct passwd *pwd = getpwnam (argv[pmr_optind]);
          if (pwd == NULL)
            error (EXIT_FAILURE, 0, _("%s: No such user"), argv[pmr_optind]);
          ruid = pwd->pw_uid;
          rgid = egid = pwd->pw_gid;

          printf ("%s : ", argv[pmr_optind]);
          if (!print_group_list (argv[pmr_optind++], ruid, rgid, egid, true))
            ok = false;
          putchar ('\n');
          // pmr: Annotation
          optind = pmr_optind;
