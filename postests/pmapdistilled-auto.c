...
int page_getfree(int_array pages)
{
    int i;
    int rv;
    int rv2;
...
int page_free(int ppno, int_array pages, int_array page_protected)
{
  assert(0 <= ppno);
  assert(ppno < 1000);
...
int is_page_protected(int ppno, int_array pages, int_array page_protected)
{
    assert(0 <= ppno);
    assert(ppno < 1000);
...
void page_decref(int ppno, int_array pages, int_array page_protected)
{
...
void env_check(env_t *env, env_t **envs, int_array pages, int_array page_protected)
{
    int i, found;
    env_t *walk;
    int ppi = 0;
...
void mem_check(env_t **envs, int_array pages, int_array page_protected)
{
    int i;
    int *lpages;
    env_t *walk;
...
env_t *env_alloc(env_t **envs, int_array pages, int_array page_protected)
{
    env_t *env;
    env_t *tmp;
...
void env_free(env_t *env, env_t **envs, int_array pages, int_array page_protected)
{
    int i;
    int ppi = 0;
