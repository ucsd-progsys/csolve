...
int main(int argc, char **argv)
{
  ...
  int i=0;
  for(; i < num_iters; ++i) {
    s.npols = pols.npols = 0;
    s.polp  = pols.polp  = NULL;
    pairs.npairs = 0;
    pairs.polpair = NULL;
    addcnt = zero = 0.0;
...
# endif
  ...
  for(vn=0;vn<nvars;vn++){
    char tbuf[32];
    scanf("%s",tbuf);
    varnames[vn] = (char*)gstralloc(sizeof(char)*(strlen(tbuf)+1));
...
# ifdef DO_GETTIME
  ...
  delta_usec = end.tv_usec - start.tv_usec;
  if (delta_usec < 0) {
