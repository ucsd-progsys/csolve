...
extern	int		pfreef();	/* free (private) */
#endif
...
#ifndef APROF  
extern inline void      pdestroyaprof(precision u, char *f, int l) {
   if (u != pUndef && --(*(prefc *) u) == 0) {
     fprintf(stderr, "%s:%d refcnt free %x\n", f, l, (unsigned int)u);
     pfreef(u);
   }
}
#else
