...
#ifdef RCSID
static char rcsid[] = "$Id: util.c 8598 2006-10-25 18:49:28Z zra $";
#endif
    ...
    unsigned n;             /* number of bytes in s[] */
{
	...
	c = crc;
        if (n) do {
            c = crc_32_tab[((int)c ^ (*scopy)) & 0xff] ^ (c >> 8);
            if(n - 1 > 0) scopy ++; // For Deputy
        } while (--n);
    }
    crc = c;
    ...
    unsigned  cnt;
{
    ...
    while ((n = write(fd, bufp, cnt)) != cnt) {
	if (n == (unsigned)(-1)) {
	    write_error();
	}
	cnt -= n;
	bufp = (voidp)((char*)bufp+n);
    }
}
...
char * NTS add_envopt(argcp, argvp, env)
    int *argcp;          /* pointer to argc */
    char * NTS * NTS *argvp;       /* pointer to argv */
    char * NTS env;           /* name of environment variable */
{
    char *p;             /* running pointer through env variable */
    char * NTS *oargv;        /* runs through old argv array */
    char * NTS *nargv;        /* runs through new argv array */
    ...
    env = strncpy(p, env, strlen(env));                    /* keep env variable intact */
    ...
    *argcp += nargc;
     ...
     */
    nargv = (char* NTS *)calloc(*argcp+1, sizeof(char *));
