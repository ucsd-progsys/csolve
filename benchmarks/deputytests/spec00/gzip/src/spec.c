...
struct spec_fd_t {
    ...
    int len;
    int pos;
    unsigned char * COUNT(limit+FUDGE_BUF) buf;
} spec_fd[MAX_SPEC_FD];
...
int spec_load (int num, char * NTS filename, int size) {
...
int spec_read (int fd, unsigned char * COUNT(size) buf, int size) {
    int rc = 0;
    debug3(4,"spec_read: %d, %x, %d = ", fd, (unsigned long)buf, size);
    if (fd > MAX_SPEC_FD) {
...
int spec_write(int fd, unsigned char * COUNT(size) buf, int size) {
  debug3(4,"spec_write: %d, %x, %d = ", fd, (unsigned long)buf, size);
    if (fd > MAX_SPEC_FD) {
