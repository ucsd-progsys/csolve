...
#ifdef RCSID
static char rcsid[] = "$Id: gzip.c 8288 2006-06-06 14:18:32Z necula $";
#endif
    ...
    int in;        /* input file descriptor */
{
    ...
    if (memcmp(magic, NTDROP(GZIP_MAGIC), 2) == 0
        || memcmp(magic, NTDROP(OLD_GZIP_MAGIC), 2) == 0) {
    ...
    } else if (memcmp(magic, NTDROP(PKZIP_MAGIC), 2) == 0 && inptr == 2
               && memcmp(/*(char*)*/inbuf, NTDROP(PKZIP_MAGIC), 4) == 0) {
    ...
    } else if (memcmp(magic, NTDROP(PACK_MAGIC), 2) == 0) {
	work = unpack;
	method = PACKED;

    } else if (memcmp(magic, NTDROP(LZW_MAGIC), 2) == 0) {
	work = unlzw;
	method = COMPRESSED;
	last_member = 1;

    } else if (memcmp(magic, NTDROP(LZH_MAGIC), 2) == 0) {
	work = unlzh;
	method = LZHED;
	last_member = 1;
