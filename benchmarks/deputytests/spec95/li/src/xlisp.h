...
//matth: turn off deputy annotations when compiling with gcc.
#include "../../../nodeputy.h"
...
#define n_symplist	n_info.n_xlist.xl_car
#define n_symvalue	n_info.n_xlist.xl_cdr
...
/* 	} n_xsym  WHEN(n_type == SYM); */
	...
	struct xlist {		/* list node (cons) matth: or symbol, too */
...
typedef struct context {
    ...
    struct node *** BND(xlstkbase, xlstktop) c_xlstack;
                                        /* old value of xlstack */
...
struct segment {
    ...
    struct node (COUNT(sg_size) sg_nodes)[0];
};
