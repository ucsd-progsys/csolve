...
#define my_cquantize_ptr jquant2_my_cquantize_ptr
...
typedef struct my_cquantizer_2 {
...
typedef my_cquantizer_2 * my_cquantize_ptr;

#pragma ccured_extends("Smy_cquantizer_2", "Sjpeg_color_quantizer")
...
/* Master routine for color selection */
{
  ...
  boxlist =
    (boxptr)alloc_small_wrapper((j_common_ptr) cinfo,
                                JPOOL_IMAGE, desired * SIZEOF(box));
...
/* Allocate and fill in the error_limiter table */
{
  ...
  table =
    (int *)alloc_small_wrapper((j_common_ptr) cinfo,
...
jinit_2pass_quantizer (j_decompress_ptr cinfo)
{
  ...
  cquantize =
    (my_cquantize_ptr)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
  ...
  cquantize->histogram =
    (hist3d)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
                                HIST_C0_ELEMS *
                                SIZEOF(*(cquantize->histogram)));
  for (i = 0; i < HIST_C0_ELEMS; i++) {
    cquantize->histogram[i] =
      (hist2d)alloc_large_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
  ...
  if (cinfo->dither_mode == JDITHER_FS) {
    ...
    cquantize->fserrors =
      (FSERRPTR)alloc_large_wrapper((j_common_ptr) cinfo,
