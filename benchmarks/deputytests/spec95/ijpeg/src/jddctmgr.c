...
typedef struct my_idct_controller {
...
#pragma ccured_extends("Smy_idct_controller", "Sjpeg_inverse_dct")
...
start_input_pass (j_decompress_ptr cinfo)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	compptr->dct_table =
	  alloc_small_wrapper ((j_common_ptr) cinfo, JPOOL_IMAGE,
...
jinit_inverse_dct (j_decompress_ptr cinfo)
{
  ...
  idct =
    (my_idct_ptr)alloc_small_wrapper ((j_common_ptr) cinfo, JPOOL_IMAGE,
