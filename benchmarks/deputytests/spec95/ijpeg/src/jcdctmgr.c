...
typedef struct my_fdct_controller {
...
#pragma ccured_extends("Smy_fdct_controller", "Sjpeg_forward_dct")
...
start_pass_fdctmgr (j_compress_ptr cinfo)
{
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
	...
	fdct->divisors[qtblno] =
          (DCTELEM *)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
...
#define CONST_BITS 14
	...
	if (fdct->float_divisors[qtblno] == NULL) {
	  fdct->float_divisors[qtblno] =
            (FAST_FLOAT *)alloc_small_wrapper((j_common_ptr) cinfo,
                                              JPOOL_IMAGE,
...
jinit_forward_dct (j_compress_ptr cinfo)
{
  ...
  fdct =
    (my_fdct_ptr)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
