	...
	for ( col = 0, pP = pixels[row]; col < cols; ++col, ++pP )
		...
		if ( ++(*colorsP) > maxcolors )
		    ...
		    return (colorhash_table) 0;
                ...
                //sm: chl = (colorhist_list) malloc( sizeof(struct colorhist_list_item) );
...
ppm_alloccolorhash( )
    ...
    //sm: cht = (colorhash_table) malloc( HASH_SIZE * sizeof(colorhist_list) );
    cht = (colorhash_table) malloc( HASH_SIZE * sizeof(*cht) );
    if ( cht == 0 )
...
ppm_colorhashtocolorhist( cht, maxcolors )
    ...
    //sm: chv = (colorhist_vector) malloc( maxcolors * sizeof(struct colorhist_item) );
