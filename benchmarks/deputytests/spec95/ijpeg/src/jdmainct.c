...
#define my_main_ptr jdmainct_my_main_ptr
...
typedef struct my_main_controller_d {
...
} my_main_controller_d;

#pragma ccured_extends("Smy_main_controller_d", "Sjpeg_d_main_controller")

typedef my_main_controller_d * my_main_ptr;
 ...
 */
{
   ...
   */
  main->xbuffer[0] =
			//sm: cinfo->num_components * 2 * SIZEOF(JSAMPARRAY));
    (JSAMPIMAGE)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
                                    cinfo->num_components *
                                    2 * SIZEOF(*(main->xbuffer[0])));
  ...
  for (ci = 0, compptr = cinfo->comp_info; ci < cinfo->num_components;
     ...
     */
    xbuf0 =
      (JSAMPARRAY)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
				  //sm: 2 * (rgroup * (M + 4)) * SIZEOF(JSAMPROW));
				  2 * (rgroup * (M + 4)) * SIZEOF(*xbuf0));
...
jinit_d_main_controller (j_decompress_ptr cinfo, boolean need_full_buffer)
{
  ...
  main =
    (my_main_ptr)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
