...
#define huff_entropy_ptr jchuff_huff_entropy_ptr
#define working_state jchuff_working_state
#define savable_state jchuff_savable_state
...
typedef struct /*sm*/savable_state {
...
typedef struct huff_entropy_encoder {
...
#pragma ccured_extends("Shuff_entropy_encoder", "Sjpeg_entropy_encoder")
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
	...
	entropy->dc_count_ptrs[dctbl] =
          (long *)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
				      257 * SIZEOF(long));
...
/* Compute the derived values for a Huffman table */
{
  ...
  if (*pdtbl == NULL)
    *pdtbl =
      (C_DERIVED_TBL *)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
...
jinit_huff_encoder (j_compress_ptr cinfo)
{
  ...
  entropy =
    (huff_entropy_ptr)alloc_small_wrapper((j_common_ptr) cinfo, JPOOL_IMAGE,
