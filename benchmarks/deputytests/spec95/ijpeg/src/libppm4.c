...
ppm_parsecolor( colorname, maxval )
	...
	if (
#ifdef CCURED
            (resetSScanfCount(colorname),
             fr = ccured_fscanf_double(ccured_sscanf_file, "rgbi:%f"),
             fg = ccured_fscanf_double(ccured_sscanf_file, "/%f"),
             fb = ccured_fscanf_double(ccured_sscanf_file, "/%f"),
             getScanfCount ())
#else
            sscanf( colorname, "rgbi:%f/%f/%f", &fr, &fg, &fb )
#endif            
            != 3 ) {
          pm_error( inval, colorname );
        }
...
#ifdef CCURED
            (resetSScanfCount(colorname),
             fr = ccured_fscanf_double(ccured_sscanf_file, "%f"),
             fg = ccured_fscanf_double(ccured_sscanf_file, ",%f"),
             fb = ccured_fscanf_double(ccured_sscanf_file, ",%f"),
             getScanfCount ())
#else
            sscanf( colorname, "%f,%f,%f", &fr, &fg, &fb )
#endif            
            != 3 )
	    pm_error( inval, colorname );
	if ( fr < 0.0 || fr > 1.0 || fg < 0.0 || fg > 1.0 || fb < 0.0 || fb > 1.0 )
	...
	while ( fgets( buf1, sizeof(buf1), f ) != NULL )
	    ...
	    if (
#ifdef CCURED
            (resetSScanfCount(buf1),
             r = ccured_fscanf_int(ccured_sscanf_file, "%ld"),
             g = ccured_fscanf_int(ccured_sscanf_file, " %ld"),
             b = ccured_fscanf_double(ccured_sscanf_file, " %ld"),
             ccured_fscanf_string(ccured_sscanf_file, " %[^\n]", buf2),
             getScanfCount ())
#else
                sscanf( buf1, "%ld %ld %ld %[^\n]", &r, &g, &b, buf2 )
#endif            
                 != 4 )
		{
		pm_message(
