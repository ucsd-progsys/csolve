...
pm_allocarray( cols, rows, size )
    ...
    // sm: the following sizeof used to not need to be sizeof (*its),
    // because we were inferring its safe (I presume).. but ok..
    //its = (char**) malloc( rows * sizeof(char*) );     // old
    its = (void**) malloc( rows * sizeof(*its) );
    if ( its == (char**) 0 )
	pm_error( "out of memory allocating an array" );
    its[0] = (void*) malloc( rows * cols * size );
    if ( its[0] == (void*) 0 )
	pm_error( "out of memory allocating an array" );
    for ( i = 1; i < rows; ++i )
...
pm_perror( reason )
    char* reason;
    {
    // sm: this conflicts with definition in stdio.h
    ...
    // sm: our tool does something funky with sys_errlist if called
    // this way; strerror(3) does something just as good..
    //e = sys_errlist[errno];     // orig
    e = strerror(errno);
