...
typedef union small_pool_struct {
  struct {
    ...
    int    padding; // GN: added because CCured cannot handle this union
  } hdr;
...
#pragma ccured_extends("Smy_memory_mgr", "Sjpeg_memory_mgr")
...
{
  // sm: untagged downcast from jpeg_memory_mgr to my_memory_mgr
  my_mem_ptr mem = (my_mem_ptr) cinfo->mem;

  small_pool_ptr hdr_ptr, prev_hdr_ptr;
  // This was char*, but is never dereferenced
  ...
  if (hdr_ptr == NULL) {
    ...
    for (;;) {
      hdr_ptr = CCURED_TRUSTEDCAST(small_pool_ptr,
                                   jpeg_get_small(cinfo, min_request + slop));
      if (hdr_ptr != NULL)
	break;
      slop /= 2;
  ...
  data_ptr = (unsigned long)hdr_ptr + sizeof(*hdr_ptr);

  data_ptr += hdr_ptr->hdr.bytes_used; /* point to place for object */
  hdr_ptr->hdr.bytes_used += sizeofobject;
  hdr_ptr->hdr.bytes_left -= sizeofobject;

  // Cast this to int first to avoid propagating the FAT constraint out of the
  // function. This is Ok because this should be called only through the
  // allocator wrapper 
...
void *alloc_small_wrapper(j_common_ptr cinfo, int pool_id,
  ...
  void * (*f)(j_common_ptr, int, size_t) = cinfo->mem->alloc_small;
  return (*f)(cinfo, pool_id, sizeofobject);
}
...
void *alloc_large_wrapper(j_common_ptr cinfo, int pool_id,
                          size_t sizeofobject) {
  // Get the function pointer
  void * (*f)(j_common_ptr, int, size_t) = cinfo->mem->alloc_large;
  return (*f)(cinfo, pool_id, sizeofobject);
}
...
/* Allocate a 2-D sample array */
{
  ...
  result = (JSAMPARRAY) alloc_small_wrapper(cinfo, pool_id,
				    //sm: (size_t) (numrows * SIZEOF(JSAMPROW)));
                                            (size_t) (numrows *
                                                      SIZEOF(*result)));
  ...
  while (currow < numrows) {
    ...
    workspace =
      (JSAMPROW) alloc_large_wrapper(cinfo, pool_id,
...
/* Allocate a 2-D coefficient-block array */
{
  ...
  result = (JBLOCKARRAY) alloc_small_wrapper(cinfo, pool_id,
				     //sm: (size_t) (numrows * SIZEOF(JBLOCKROW)));
                                             (size_t) (numrows * SIZEOF(*result)));
...
jinit_memory_mgr (j_common_ptr cinfo)
{
  ...
  /* But make sure we have the right RTTI */
  // was cinfo->mem = & mem->pub'
  cinfo->mem = (struct jpeg_memory_mgr *) mem;
  ...
  { char * memenv;
    ...
    if ((memenv = getenv("JPEGMEM")) != NULL) {
      ...
      if (
#ifdef CCURED
          (resetSScanfCount(memenv),
           max_to_use = ccured_fscanf_int(ccured_sscanf_file, "%ld"),
           ch = ccured_fscanf_int(ccured_sscanf_file, "%c"),
           getScanfCount())
#else
          sscanf(memenv, "%ld%c", &max_to_use, &ch)
#endif
          > 0) {
	if (ch == 'm' || ch == 'M')
	  max_to_use *= 1000L;
