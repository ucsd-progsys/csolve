...
jpeg_create_compress (j_compress_ptr cinfo)
{
   ...
   */
  {
    ...
    //sm: MEMZERO(cinfo, SIZEOF(struct jpeg_compress_struct));
    MEMZERO(cinfo, SIZEOF(*cinfo));
    cinfo->err = err;
  }
  cinfo->is_decompressor = FALSE;

  /* Initialize a memory manager instance for this object */
  jinit_memory_mgr((j_common_ptr) TC(cinfo));
...
{
  jpeg_destroy((j_common_ptr) TC(cinfo)); /* use common routine */
}
...
jpeg_start_compress (j_compress_ptr cinfo, boolean write_all_tables)
{
  ...
  (*cinfo->err->reset_error_mgr) ((j_common_ptr) TC(cinfo));
		      ...
		      JDIMENSION num_lines)
{
  ...
  if (cinfo->progress != NULL) {
    ...
    (*cinfo->progress->progress_monitor) ((j_common_ptr) TC(cinfo));
  }
  ...
  /* if you want origional erase the next line and restor the following comment*/
  #ifdef DANNY
    row_ctr = 739;
  #else
    row_ctr = 0;
...
{
  JDIMENSION iMCU_row, mcu_ctr __HEAPIFY ;
  ...
  jpeg_abort((j_common_ptr) TC(cinfo));
}
...
{
  jpeg_abort((j_common_ptr) TC(cinfo)); /* use common routine */
}
