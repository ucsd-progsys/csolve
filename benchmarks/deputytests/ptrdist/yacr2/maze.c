...
// Introduce a global to remember (channelColumns+1)*(channelTracks+3)
static ulong  ASSUMECONST     channelProduct; 


static char * COUNT(channelProduct) horzPlane;	/* horizontal plane allocation map */

/* r/lvalue for accessing horizontal plane allocation map */
#define HORZ(x,y)	ACCESS_MAP(horzPlane, x, y)

static char * COUNT(channelProduct) vertPlane;	/* vertical plane allocation map */

/* r/lvalue for accessing vertical plane allocation map */
#define VERT(x,y)	ACCESS_MAP(vertPlane, x, y)

static char * COUNT(channelProduct) viaPlane;		/* via plane allocation map */

/* r/lvalue for accessing via plane allocation map */
#define VIA(x,y)	ACCESS_MAP(viaPlane, x, y)

static char * COUNT(channelColumns+1) mazeRoute;	/* true if the col needs to be maze routed */
...
{
    /* allocate maps */
    channelProduct = (channelColumns + 1) * (channelTracks + 3);
 ...
 */
void
DrawSegment(char * COUNT(channelProduct) plane,
 ...
 */
int
SegmentFree(char * COUNT(channelProduct) NONNULL plane,
	    unsigned long x1, unsigned long y1,
	    unsigned long x2, unsigned long y2)
{
    unsigned long x, y;
 ...
 */
int
ExtendOK(unsigned long net,
         char * COUNT(channelProduct) plane,
