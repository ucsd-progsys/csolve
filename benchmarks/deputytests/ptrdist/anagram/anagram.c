...
typedef struct {
    ...
    char * NT COUNT(cchLength) pchWord;                 /* the word itself */
...
char * NT BND(__this, __auto) pchDictionary;               /* the dictionary is read here */

#define Zero(t) memset(t, 0, sizeof(t)) /* quickly zero out an integer array */

/* Fatal -- print a message before expiring */
void Fatal(char * NTS pchMsg, unsigned u) {
  fprintf(stderr, pchMsg, u); //GN: format-string bug lurking
  exit(1);
}
...
void ReadDict(char * NTS pchFile) {
    FILE *fp;
    char * pch;
    char * pchBase;
    unsigned long ulLen;
...
void BuildMask(char * NT COUNT(ACH_PHRASE_SIZE -1) pchPhraseArg) {

    char * pchPhrase = pchPhraseArg; // For Deputyl, so we can ++
 ...
 */
void BuildWord(char * NTS pchWord) {
...
void
FindAnagram(Quad * COUNT(MAX_QUADS) pqMask,
            PPWord BND(__this, &apwCand[MAXCAND]) ppwStart, int iLetter)
{
    Quad aqNext[MAX_QUADS];
    register PWord pw;
    Quad qMask;
    unsigned iq;
    Word * * ppwEnd = &apwCand[0];
    ppwEnd += cpwCand;
...
void SortCandidates(void) {
    ...
    { //__NOCUREBLOCK
      qsort(achByFrequency, ALPHABET, sizeof(char),
           /*(int (*)(const void *, const void *))*/ TC(CompareFrequency));
    }
...
// It seems that GetPhrase is called with a 255 byte buffer here. Ideally, we
// should add an argument that specifies the length 
char * NT COUNT(ACH_PHRASE_SIZE - 1)
GetPhrase(char *  NT COUNT(ACH_PHRASE_SIZE - 1) pch) {
    if (fInteractive) printf(">");
    fflush(stdout);
    if (fgets(pch, ACH_PHRASE_SIZE, stdin) == NULL) { // gn: replaced gets with fgets
#ifdef PLUS_STATS
...
char (NT achPhrase)[ACH_PHRASE_SIZE];
...
int Cdecl main(int cpchArgc, char * NTS * NT COUNT(cpchArgc) ppchArgv) {
