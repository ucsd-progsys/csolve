...
/* From util.c */
_PROTOTYPE(char * NTS strcopyof, (char * NTS str));
_PROTOTYPE(arg_list * nextarg, (arg_list * args, char val));
_PROTOTYPE(char * NTS arg_str, (arg_list * args, int));
...
_PROTOTYPE(void generate, (char * NTS str));
_PROTOTYPE(void run_code, (void));
_PROTOTYPE(void out_char, (char ch));
_PROTOTYPE(id_rec * find_id, (id_rec * tree, char * NTS id));
_PROTOTYPE(int insert_id_rec, (id_rec * * root, id_rec * new_id));
_PROTOTYPE(void init_tree, (void));
_PROTOTYPE(int lookup, (char * NTS name, int namekind));
#ifdef notdef
_PROTOTYPE(char *bc_malloc, (int));
_PROTOTYPE(void out_of_memory, (void));
#endif
_PROTOTYPE(void welcome, (void));
_PROTOTYPE(void warranty, (char * NTS));
_PROTOTYPE(void limits, (void));
_PROTOTYPE(void (DPRINTF(1) yyerror), (char * NTS str ,...));
_PROTOTYPE(void (DPRINTF(1) warn), (char * NTS mesg ,...));
_PROTOTYPE(void (DPRINTF(1) rt_error), (char * NTS mesg ,...));
_PROTOTYPE(void (DPRINTF(1) rt_warn), (char * NTS mesg ,...));

/* From load.c */
_PROTOTYPE(void init_load, (void));
_PROTOTYPE(void addbyte, (int byte));
_PROTOTYPE(void def_label, (long lab));
_PROTOTYPE(long long_val, (char * NTS * SAFE str));
_PROTOTYPE(void load_code, (char * NTS code));

/* From main.c */
_PROTOTYPE(int main, (int argc , char * NTS * NT COUNT(argc) argv));
_PROTOTYPE(int open_new_file, (void));
_PROTOTYPE(void new_yy_file, (FILE * SAFE file));
_PROTOTYPE(void use_quit, (int));

/* From number.c */
_PROTOTYPE(void free_num, (bc_num * SAFE NONNULL num));
...
_PROTOTYPE(void str2num, (bc_num * num, char * NTS NONNULL str, int scale));
_PROTOTYPE(char * NTS NONNULL num2str, (bc_num num));
