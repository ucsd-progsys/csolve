...
/* It seems that sometimes there is some room before the result */
_PROTOTYPE(static void _one_mult,
           (char * COUNT(size)  num, int size, 
            int digit, char * BND(__this - extra_first,
                                  __this + size) result,
            int extra_first));
  ...
  while ((n1bytes > 0) && (n2bytes > 0))
    {
      *sumptr = *n1ptr + *n2ptr + carry;
      if(n1bytes - 1 > 0) n1ptr --; // For Deputy
      if(n2bytes - 1 > 0) n2ptr --; // For Deputy
      if (*sumptr > 9)
	{
	   carry = 1;
	   *sumptr -= 10;
...
_do_sub (bc_num n1, bc_num n2)
{
  ...
  for (count = 0; count < min_len + min_scale; count++)
	...
	borrow = 0;
      *diffptr = val;
      // Now do the decrements for Deputy, if we stay in the loop
      if(count + 1 < min_len + min_scale) {
        n2ptr --; n1ptr --; diffptr --;
      } 
    }
  ...
  if (diff_len != min_len)
    {
      // Do the decrements that we missed above
      n1ptr --; diffptr --;
...
bc_add ( bc_num n1, bc_num n2, bc_num * result)
{
  bc_num sum;
  int cmp_res;
  sum = 0; //Deputy wants us to initialize sum because it doesn't know
           //know that do_compare always returns -1, 0, or 1.
...
bc_multiply (bc_num n1, bc_num n2, bc_num * prod, int scale)
{
  ...
  for (indx = 0; indx < toss; indx++)
	...
	sum += *n1ptr-- * *n2ptr++;
        //if(n1ptr > n1->n_value) n1ptr --; // conditional for Deputy
      }
      sum = sum / 10;
    }
  for ( ; indx < total_digits-1; indx++)
    { // To make Deputy happy we cannot decrement the pointer before the area
      // So, we add another termination flag
      int done = 0;
      n1ptr = n1end - MAX(0, indx-len2+1);
      n2ptr = n2end - MIN(indx, len2-1);
      
      while (!done && (n1ptr >= n1->n_value) && (n2ptr <= n2end)) {
	sum += *n1ptr * *n2ptr++;
        if(n1ptr > n1->n_value) {
          n1ptr --;
        } else
...
/* Deputy: It seems that sometimes there is some extra room before result */
static void
_one_mult (char * COUNT(size) num, int size,
           int digit,
	   char * BND(__this - extra_first, __this + size) result,
           int extra_first)
{
  int carry, value;
  char * nptr;
  ...
  if (digit == 0)
	...
	{
          int sizec = size; // Make a copy for Deputy
	  /* Initialize */
	  nptr = num+sizec-1;
	  rptr = result+sizec-1;
	  carry = 0;
	  ...
	  while (sizec-- > 0)
	    {
	      ...
	      *rptr = value % 10;
	      carry = value / 10;
              if(sizec > 0) { nptr --; rptr --; } // For Deputy
	    }
          
	  if (carry != 0) * --rptr = carry; // To account for missing -- above
	}
    }
}
...
{ 
  ...
  scale2 = n2->n_scale;
  n2ptr = &n2->n_value[0] + n2->n_len + scale2 - 1;
  ...
  if (!zero)
    {
      ...
      if (norm != 1)
	{
	  _one_mult (num1, len1+scale1+extra+1, norm, num1, 0);
	  _one_mult (n2ptr, len2, norm, n2ptr, 0);
	}
      ...
      while (qdig <= len1+scale-len2)
	{
	  ...
	  if (qguess != 0)
	    {
	      *mval = 0;
	      _one_mult (n2ptr, len2, qguess, mval+1, 1);
	      ptr1 = num1+qdig+len2;
	      ptr2 = mval+len2;
	      for (count = 0; count < len2+1; count++)
		{
		  val = (int) *ptr1 - (int) *ptr2 - borrow;
                  if(count + 1 < len2 + 1) ptr2 --; // For Deputy
		  if (val < 0)
		    {
		      val += 10;
		      borrow = 1;
		    ...
		    borrow = 0;
		  *ptr1 = val;
                  if(count + 1 < len2 +1) ptr1 --; // For Deputy
		}
	    }
	  ...
	  if (borrow == 1)
	      ...
	      for (count = 0; count < len2; count++)
		{
		  ...
		  if (val > 9)
		    ...
		    carry = 0;
		  *ptr1-- = val;
                  // Do the deferred decrement
                  if(count + 1 < len2) { ptr2 --; }
		}
...
{
   bc_num temp, power; // So that we can take address-of
   long exponent;
   int rscale;
   char neg;
   ;
...
bc_sqrt (bc_num * num, int scale)
{
  ...
  int cscale;
  bc_num guess, guess1, point5; // To take the address
  ;
