...
#if defined(DEPUTY)
// Add one more element to the array so we can point to -1
YYSTYPE yyv[1+YYMAXDEPTH]; /* where the values are stored */
#define yyvoffset (&yyv[1])
#else
YYSTYPE yyv[YYMAXDEPTH]; /* where the values are stored */
#define yyvoffset yyv
#endif
...
yyparse() {
#if defined(DEPUTY)
  // Add one more element at the start of yys
        short yys[1+YYMAXDEPTH];
  // Define an offset array        
#define yysoffset (&yys[1])
#else        
	short yys[YYMAXDEPTH];
#define yysoffset yys
#endif        
	short yyj, yym;
...
#endif
		if( ++yyps> &yysoffset[YYMAXDEPTH] ) { yyerror( "yacc stack overflow" ); return(1); }
		*yyps = yystate;
		++yypv;
		*yypv = yyval;
