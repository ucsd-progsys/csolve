...
/* sm: little thunk to eliminate the bad cast */
void out_char_int(int i)
{
  out_char((char)i);
}
  ...
  while (pc.pc_addr < functions[pc.pc_func].f_code_size && !runtime_error)
	...
	break;
      ...
      case 'P' : /* Write the value on the top of the stack.  No newline. */
        //sm: removed: out_num (ex_stack->s_num, o_base, (void (*)(int))out_char);
	out_num (ex_stack->s_num, o_base, out_char_int);
	if (inst == 'W') out_char ('\n');
