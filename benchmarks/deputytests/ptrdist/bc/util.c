...
static char * NTS arglist1;
static char * NTS arglist2;
...
make_arg_str (arg_list * args, int len, int commas)
{
  ...
  if (len != 1 && commas) 
    ...
    sprintf (sval, "%d", args->av_name);
  strncat(temp, sval, len - strlen(temp)); // For Deputy
  return (temp);
}
...
int
lookup (char * NTS name, int namekind)
{
  id_rec * id;
