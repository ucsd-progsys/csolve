...
static void unquantize_image(BinIndexType * COUNT(im_size) NONNULL q_im,
                            int * COUNT(im_size) NONNULL res, int im_size,
                            BinValueType bin_size);

static void quantize_image (float * COUNT(im_size) NONNULL float_im,
                           BinIndexType * COUNT(im_size) NONNULL q_im,
                           int im_size, BinValueType *bin_size);
...
void quantize_pyr (float_pyr, q_pyr, im_size, num_levels, compression_factor, bin_size)
  float *float_pyr;
  BinIndexType *q_pyr;
  BinValueType *bin_size;
  ...
  int tim_size = im_size; // So that we can /=
  
  max_bin_size = ~max_bin_size;   /* all 1's */

  for (tim_size/=4, level=1, bin_index=3*num_levels;
       level <= num_levels; 
       level++, tim_size /= 4, the_bin_size /= 2.0)
    for (im_num = 3, pyr_offset = 3*tim_size;
	 im_num >= ((level == num_levels) ? 0 : 1);
	 im_num--, pyr_offset-=tim_size, bin_index--)
	{
	...
	quantize_image(float_pyr+pyr_offset, q_pyr+pyr_offset, tim_size, 
		       (bin_size+bin_index));
...
static void quantize_image (float_im, q_im, im_size, bin_size)
...
void unquantize_pyr(q_pyr, pyr, im_size, num_levels, bin_size)
  BinIndexType *q_pyr;
  int *pyr, im_size, num_levels;
  BinValueType *bin_size;
  {
  int level, bin_index, im_num, pyr_offset = 0;
  int tim_size = im_size; // So we can >=
  
  for (tim_size>>=(num_levels*2), level=num_levels, bin_index=0;
       level > 0;
       level--, tim_size<<=2)
	...
	unquantize_image((q_pyr+pyr_offset), (pyr+pyr_offset), tim_size, 
			 bin_size[bin_index]);
	}
  }
...
static void unquantize_image(q_im, res, im_size, bin_size)
  register BinIndexType *q_im;
