...
static int internal_transpose(float * COUNT(rows*cols) NONNULL mat,
                              int rows, int cols);

static int build_level(float * COUNT(level_x_size*level_y_size) NONNULL image,
                       int level_x_size, int level_y_size,
                       float * COUNT(filter_size) NONNULL lo_filter,
                       float * COUNT(filter_size) NONNULL hi_filter,
	               int filter_size,
                       float * COUNT(level_x_size*level_y_size) NONNULL result_block);
...
======================================================================
*/
int build_pyr(image, x_size, y_size, num_levels, lo_filter, hi_filter, filter_size)
...
======================================================================
*/
static int build_level(image, level_x_size, level_y_size, lo_filter, hi_filter,
  ...
  float * tresult_block = result_block; // So we can +=
  ...
  internal_filter (image, level_x_size, level_y_size, 
		   ...
		   0, 2, 0, 1, lo_imagetemp, "reflect1"); // Bug
  internal_filter (image, level_x_size, level_y_size, 
		   hi_filter, filtertemp, filter_size, 1, 
		   1, 2, 0, 1, hi_imagetemp, "reflect1"); // Bug
...
======================================================================
*/
static int internal_transpose( mat, rows, cols )
  register float *mat;
  register int cols;
  int rows;
  {
