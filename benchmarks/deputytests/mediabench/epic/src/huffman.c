...
struct node
  ...
  SymbolType symbol;
  char * NTS code;                           /* a string of 0's and 1's */
  };

static void compute_histogram(SymbolType * COUNT(symbol_stream_length) NONNULL symbol_stream,
                             int symbol_stream_length,
                             struct node * * COUNT(NUM_SYMBOL_VALUES) NONNULL histogram);
static int encode_stream(SymbolType * COUNT(symbol_stream_length) NONNULL symbol_stream,
                         int symbol_stream_length,
                         struct node * * COUNT(NUM_SYMBOL_VALUES) NONNULL histogram,
                         CodeType * COUNT(symbol_stream_length) NONNULL encoded_stream);

static void build_huffman_tree(struct node * * COUNT(NUM_SYMBOL_VALUES) NONNULL histogram,
                               struct node * * NONNULL tree_ptr);

static int pack_huffman_tree(struct node * tree, Byte * EFAT * packed_tree);
static void pack_tree_iter(struct node * tree, Byte * EFAT * packed_tree_ptr);
static void compute_code_strings(struct node * tree,
                                 char * NT COUNT(NUM_SYMBOL_VALUES-1) NONNULL code_string,
                                 int code_string_pos);
...
static void compute_histogram(symbol_stream, symbol_stream_length, histogram)
  ...
  SymbolType * tsymbol_stream = symbol_stream; // For ++
  
  for (i=0; i<symbol_stream_length; i++, tsymbol_stream++)
      {
      the_leaf = histogram[ *tsymbol_stream ];  /* index into histogram from symbol */
      if (the_leaf == NULL)
	  {
	  the_leaf = histogram[ *tsymbol_stream ] =
	    (struct node *) check_malloc(sizeof(struct node));
	  the_leaf->symbol = *tsymbol_stream;
...
static void build_huffman_tree(histogram, tree_ptr)  
  register struct node *histogram[];
...
static void compute_code_strings(struct node *tree,
  ...
  if ( tree->zero_child == NULL )    /* If leaf */
      {
        char * res = check_malloc(code_string_pos+1);
        res[code_string_pos] = '\0';
        code_string[code_string_pos] = '\0';    /* terminate string */ 
        tree->code = strncpy(res,
                             code_string, code_string_pos);
      }
  else
      {
...
void read_and_huffman_decode(file, tree, symbol_stream, count)
  FILE *file;
...
bits in CodeType */
static int encode_stream(symbol_stream, symbol_stream_length, histogram, encoded_stream)
  SymbolType *symbol_stream;
  int symbol_stream_length;
  ...
  *code_pointer = 0;
  for (i=0; i<symbol_stream_length; i++, tsymbol_stream++)
      {
      the_node = histogram[ *tsymbol_stream ];
      if (the_node == NULL) 
...
static int pack_huffman_tree(tree, packed_tree)
  struct node *tree;
  Byte * * packed_tree;
  {
  Byte * EFAT packed_tree_ptr;
  int num_leaves;
...
static void pack_tree_iter(tree, packed_tree_ptr)
  struct node *tree;
  Byte * EFAT * packed_tree_ptr;
  {
...
free_tree_nodes(tree)
  ...
  else check_free (/*(char *)*/ TC(tree->code));
  check_free (/*(char *)*/ TC(tree));
  return 1;
  }
  
void print_list_nodes(tree,flag)
  struct node *tree;
  int flag;
  {
