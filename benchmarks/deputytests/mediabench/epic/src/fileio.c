...
void read_byte_image(stream, image, x_size, y_size)
  FILE *stream;
  ...
  if (status != im_size) { printf ("Error reading byte image file (status=%d, im_size=%d).\n", status, im_size); exit(-1); }
...
float *ReadMatrixFromPGMStream(Infile, xsize, ysize)
  ...
  if (M == NULL) {
    ...
    fclose (Infile); // Bug: was close instead of fclose
    exit(-1);
  }
...
void write_byte_image(stream, image, x_size, y_size)
  FILE *stream;
  register int *image;
  int x_size, y_size;
  {
...
void write_pgm_image(stream, image, x_size, y_size)
  FILE *stream;
  register int *image;
  int x_size, y_size;
  {
