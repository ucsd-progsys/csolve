...
int run_length_encode_zeros (BinIndexType *block, int block_size,
                             SymbolType *encoded_stream)
  {
  ...
  while (block_pos < block_size)
	  ...
	  {
            // There was a bug in here. The block_pos < block_size was missing
	  for (count=0; block_pos < block_size && block[block_pos]==0; count++) block_pos++;
...
void run_length_decode_zeros(symbol_stream, block_size, block)
  BinIndexType *block;
  int block_size;
