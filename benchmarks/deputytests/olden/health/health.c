...
typedef struct fc_pointer {
  struct Village *value; } future_cell_pointer;
typedef struct {
  struct List *value; } future_cell_pointer_List;
typedef struct fc_results {
...
{  
  if (level == 0) 
    ...
    for (i = 3; i > 0; i--)
      fval[i].value = 
	alloc_tree(level - 1, lo + (proc*(i))/4, 
					proc / 4, (label * 4) + i + 1, new); 
#endif

    fval[0].value = alloc_tree(level - 1, lo, proc / 4, 
						    (label * 4) + 1, new);    
    new->back = back;
    NOTEST();
    new->label = label;
...
{ 
  ...
  timer_start(0);
  //BUG: This used to use top before defining it:
  //top = alloc_tree(max_level, 0, __NumNodes, 0, top);
  top = alloc_tree(max_level, 0, __NumNodes, 0, 0);
...
struct List *sim(struct Village *village)
{
  ...
  struct Village         *v;
  struct Hosp            *h;
  future_cell_pointer_List    val[4];
