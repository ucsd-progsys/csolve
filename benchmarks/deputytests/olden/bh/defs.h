...
#define TypeB(x) (((nodeptr) &(x)->bodynode)->type)
#define MassB(x) (((nodeptr) &(x)->bodynode)->mass)
#define PosB(x)  (((nodeptr) &(x)->bodynode)->pos)
#define NextB(x) (((nodeptr) &(x)->bodynode)->next)
...
#endif
	nodeptr SAFE  subp[NSUB];     /* descendents of cell */
	matrix quad;            /* quad. moment of cell */
    } stuff;
} cell, *cellptr;
 
#define TypeC(x) (((nodeptr) &(x)->cellnode)->type)
#define MassC(x) (((nodeptr) &(x)->cellnode)->mass)
#define PosC(x)  (((nodeptr) &(x)->cellnode)->pos)
#define NextC(x) (((nodeptr) &(x)->cellnode)->next)
...
void *(DALLOC(nb) allocate)(int nb);		/* allocate and zero memory */
real distv(vector, vector);			/* distance between vectors */
void (DPRINTF(1) error)(string, ...);		/* report error and exit    */
void (DPRINTF(1) eprintf)(string, ...);		/* printf to error stream   */
