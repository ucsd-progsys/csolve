...
local void pickshell(vectorparam, real);	/* pick point on shell      */
...
local void testdata(void)
{
	...
	TypeB(p) = BODY;				/*   tag as a body          */
	MassB(p) = 1.0 / nbody;			/*   set masses equal       */
	r = 1 / rsqrt(rpow(xrandom(0.0, MFRAC),	/*   pick r in struct units */
			 -2.0/3.0) - 1);
	pickshell(PosB(p), rsc * r);		/*   pick scaled position   */
	ADDV(cmr, cmr, PosB(p));		/*   add to running sum     */
    ...
    for (p = bodytab; p < bodytab+nbody; p++) {	/* loop over particles      */
	SUBV(PosB(p), PosB(p), cmr);		/*   offset by cm coords    */
...
local void stepsystem(void)
{
	...
	for (p = bodytab; p < bodytab+nbody; p++) {
	    ...
	    hackgrav(p, MassB(p) > 0.0);		/*     get force on each    */
	...
	ADDV(PosB(p), PosB(p), dpos);		/*   advance r by 1 step    */
    }
    maketree(bodytab, nbody);			/* build tree structure     */
    nfcalc = n2bcalc = nbccalc = 0;		/* zero counters            */
