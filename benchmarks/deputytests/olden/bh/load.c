...
void maketree(bodyptr COUNT(nbody) btab, int nbody)
{
    ...
    threadtree((nodeptr) &root->cellnode, NULL);           /* add Next and More links  */
...
local cellptr makecell(void)
{
    ...
    if (freecell == NULL)                       /* no free cells left?      */
      ...
      c = TC(freecell);        /*   take one on front      */
        freecell = NextC(c);                     /*   go on to next one      */
    }
    TypeC(c) = CELL;                             /* initialize cell type     */
...
local void expandbox(bodyptr COUNT(nbody) btab, int nbody)
{
    real xyzmax;
    bodyptr p;
    int k;
	...
	for (k = 0; k < NDIM; k++)
	    xyzmax = MAX(xyzmax, rabs(PosB(p)[k] - PosC(root)[k]));
...
local void loadbody(bodyptr p)
{
  ...
  bodyptr SAFE b;
    ...
    while (Subp(q)[qind] != NULL) {             /* loop descending tree     */
		...
		PosC(c)[k] = PosC(q)[k] +		/*       offset from parent */
		    (PosB(p)[k] < PosC(q)[k] ? - qsize : qsize) / 4;
            b = TC((Subp(q)[qind]));
            Subp(c)[subindex(b, c)] = Subp(q)[qind];
                                                /*     put body in cell     */
            Subp(q)[qind] = (nodeptr) &(c->cellnode);        /*     link cell in tree    */
        }
        q = TC(Subp(q)[qind]);		/*   advance to next level  */
	qind = subindex(p, q);			/*   get index to examine   */
        qsize = qsize / 2;                      /*   shrink current cell    */
        lev++;                                  /*   count another level    */
    }
    Subp(q)[qind] = (nodeptr) &p->bodynode;                /* found place, store p     */
...
local int subindex(bodyptr p, cellptr q)
{
    ...
    for (k = 0; k < NDIM; k++)			/*   loop over dimensions   */
        if (PosC(q)[k] <= PosB(p)[k])		/*     if beyond midpoint   */
...
local void hackcofm(cellptr p, real psize)
{
	...
	if ((q = Subp(p)[i]) != NULL) {         /*   does subnode exist?    */
	    if (Type(q) == CELL) {		/*     and is it a cell?    */
              cellptr SAFE c = TC(q);
              hackcofm(c, psize/2); /*       find subcell cm    */
            }
	    MassC(p) += Mass(q);                 /*     sum total mass       */
	    MULVS(tmpv, Pos(q), Mass(q));       /*     weight pos by mass   */
	    ADDV(cmpos, cmpos, tmpv);           /*     sum c-of-m position  */
	}
    DIVVS(cmpos, cmpos, MassC(p));               /* rescale cms position     */
    for (k = 0; k < NDIM; k++)			/* check tree structure...  */
	if (cmpos[k] < PosC(p)[k] - psize/2 ||   /*   if out of bounds       */
	      PosC(p)[k] + psize/2 <= cmpos[k])	/*   in either direction    */
	    error("hackcofm: tree structure error\n");
    setrcrit(p, cmpos, psize);                  /* set critical radius      */
    SETV(PosC(p), cmpos);			/* and center-of-mass pos   */
}

/*
...
local void setrcrit(cellptr p, vectorparam cmpos, real psize)
{
    real rc, bmax2, dmin;
    int k;
	...
	rc = psize / theta + distv(cmpos, PosC(p));
...
local void threadtree(nodeptr p, nodeptr n)
{
    ...
    if (Type(p) == CELL) {                      /* any children to thread?  */
      cellptr SAFE c = TC(p);
        ...
        for (i = 0; i < NSUB; i++)              /*   loop over subnodes     */
            if (Subp(c)[i] != NULL)             /*     found a live one?    */
                desc[ndesc++] = Subp(c)[i];     /*       store in table     */
        More(c) = desc[0];                      /*   link to first child    */
...
local void hackquad(cellptr p)
{
	...
	if ((q = psub[i]) != NULL) {		/*   does subnode exist?    */
	    if (Type(q) == CELL){		/*     and is it a call?    */
              cellptr SAFE c = TC(q);
		hackquad(c);		/*       process it first   */
            }
	    SUBV(dr, Pos(q), PosC(p));           /*     displacement vect.   */
	    ...
	    if (Type(q) == CELL)  {              /*     if subnode is cell   */
              cellptr SAFE c = TC(q);
              ADDM(tmpm, tmpm, Quad(c));      /*       add its moment     */
            }
