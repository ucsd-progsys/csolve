...
local string * NTS defaults = NULL;			/* "name=value" strings     */

/*
 * INITPARAM: ignore arg vector, remember defaults.
 */

void initparam(string * NTS argv, string * NTS defv)
{
    defaults = defv;
}
...
string getparam(string name)
{
    ...
    { //matth: changed for deputy, because gets is unsound.
      //gets(buf);				/* read users response      */
      fgets((char*)buf, 128, stdin);		/* read users response      */
      len = strlen(buf);
      if ((len > 0) && (buf[len-1] == '\n')) {  
        //fgets includes the newline.  Delete it to match gets's behavior
        buf[len-1] = 0;
        len--;
      }
    }; //end deputy changes
    if (len > 0)				/* if user gave a value...  */
	return (strncpy((string) allocate(len+1), buf, len));
...
local int scanbind(string * NTS bvec, string name)
{
    int i;
