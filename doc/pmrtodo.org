...
* TODO Remember to commit fixpoint
* TODO Add fixInterface code for ensuring that effects are disjoint

* TODO Test with loops, ifs
* TODO Test with function pointers
* TODO Add parallel blocks
* TODO How does foreach work?
  We need to show that two iterations of the foreach necessarily touch
  different parts of memory, right? So i, j in the limits and not equal,
  instantiate the effects, show that they're different.
